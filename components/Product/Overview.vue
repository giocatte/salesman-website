<template>
    <div id="Overview_Component" v-if="products && products.length > 0" class="flex flex-col gap-y-2 px-2 mb-8">
        ...
        <div class="flex items-end gap-1 overflow-x-auto no-scrollbar min-h-11">
            <div class="w-fit m-1 px-[1px] pb-1 bg-BlueToRed rounded-full" 
                v-for="p in products" :key="p.Id" 
                @click="setActiveTab(p.component.Title)">
                <button class="h-fit -m-[1px] p-2 text-black transition-all duration-150 rounded-full" 
                    :class="{
                        'bg-white font-bold': activeTab === p.component.Title, 
                        'hover:bg-white bg-white': activeTab !== p.component.Title
                    }">
                    {{ p.component.BtnText }}
                </button>
            </div>
        ...
        <div v-for="p in products" :key="p.Id" 
            v-show="p.component.Title === activeTab" 
...
watch(() => props.products, (newProducts) => {
    if (newProducts && newProducts.length > 0) {
        activeTab.value = newProducts[0].component.Title; // Imposta il primo tab
    }
}, { immediate: true });
