<template>
  <!-- -------------------- MOBILE ----------------------------------- -->
  <div v-if="products?.length && isMobile" id="Overview_Component" class="w-full flex flex-col gap-y-2 px-4 mb-8">
    <p class="mt-2 p-2 text-[2.5rem] font-nunito font-semibold">Servizi</p>

    <!-- Tab bar -->
    <div class="overflow-x-scroll flex items-end gap-x-1 min-h-10 snap-x snap-proximity">
      <div v-for="p in products" :key="p.Id" class="p-1 mb-2 rounded-full transition-all"
        :class="{ 'bg-BlueToRed': activeTab === p.Name }" @click="setActiveTab(p)">
        <button class="px-2 py-1 rounded-full text-black"
    ...
    <div v-for="p in products" :key="p.Id" v-show="p.Name === activeTab"
      ...
      <!-- Immagine -->
      <div class="relative self-center w-full max-w-[400px] rounded-[1.25rem] bg-cover bg-center"
  ...
  <div v-else-if="products?.length && isDesktop">
    <div class="relative w-full h-fit
               ...
               flex content-center">
      <!-- COLONNA TESTO -------------------------------------------- -->
      <div v-if="comp && compPrev && compNext" class="relative w-full lg:w-7/12 flex flex-col gap-y-5">
        <!-- Titolo sezione -->
        ...
        <div class="overflow-x-auto flex items-center gap-x-4">
          <div v-for="p in products" :key="p.Id" class="py-1 rounded-full transition-all"
            :class="{ 'bg-BlueToRed px-1': activeTab === p.Name }" @click="setActiveTab(p)">
            <button class="bg-white rounded-full px-2 py-[1px] text-black font-bold
        ...
        <!-- Titolo prodotto -->
        <p class="font-nunito font-semibold
                   lg:h2-Tablet xl:h2-Desktop text-gr" v-html="desktopTitle" />

        <!-- Descrizione -->
        <p class="font-nunito font-semibold
                   lg:h3-Tablet xl:h3-Desktop text-gr" v-html="comp.Description" />

        <!-- Bottone -->
        <NuxtLink :to="`/Servizi/${comp.BtnText}`" id="ChiSono" class="p-1 w-fit rounded-2xl bg-BlueToRed transition-all">
          <button class="bg-white rounded-xl font-bold
      ...
      <div class="hidden lg:flex lg:w-5/12 relative justify-center">
        <img :src="compPrev.ImgUrl" class="absolute top-10 left-6 lg:left-6 xl:left-10 z-0 blur-sm rounded-3xl
                   h-[18rem] w-[18rem] lg:h-[23.8rem] lg:w-[23.8rem]" />
        <img :src="comp.ImgUrl" id="compImg" :class="[
        ...
        <img :src="compNext.ImgUrl" class="absolute top-10 right-0 z-0 blur-sm rounded-3xl
                   h-[18rem] w-[18rem] lg:h-[23.8rem] lg:w-[23.8rem]" />
      </div>
    </div>
  </div>
