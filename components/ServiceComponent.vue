<template>
    <div id="ServiceComponent" class="flex flex-col gap-y-2 p-6 font-nunito">
        <!-- Tabs -->
        <div class="flex items-end gap-2 border-b border-gray-300 overflow-x-auto no-scrollbar min-h-14">
            <button v-for="s in services" :key="s.Title" @click="setActiveTab(s.Title)"
                class="mx-2 my-1 h-fit px-2 text-gray-600 transition-all duration-100 rounded-2xl" :class="{
                    'bg-white text-black font-semibold border-2 border-black': activeTab === s.Title, 'hover:bg-white': activeTab !== s.Title
                }">
                {{ s.BtnText }}
            </button>
        </div>
        ...
        <div v-for="s in services" :key="s.Title" v-show="s.Title === activeTab"
            ...
            <!-- Title -->
            <p class="text-3xl font-semibold" v-html="s.ImgUrl"></p>

            <!-- Description -->
            <p class="text-base text-justify desc" v-html="s.Description"></p>

            <!-- Button -->
            <button class="text-black w-fit self-start mt-2 p-2 border-[3px] border-black shadow-lg rounded-full font-inter font-bold">SCOPRI DI PIU'</button>
        </div>
    </div>
</template>
